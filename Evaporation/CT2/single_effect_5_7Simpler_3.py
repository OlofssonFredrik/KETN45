...
def evaporator(X):
    ...
    #K-values
    [k1,dummy1]=k_black_liquor(xF, xL1, T1, F, L1)
    [k2,dummy1]=k_black_liquor(xL1, xL2, T2, L1, L2)
    [k3,dummy1]=k_black_liquor(xL2, xL3, T3, L2, L3)
    ...
    Y[3]=k1*A*(Ts-T1) - S*(Hs-hk1)             #EB för heat ex 1
    
    #evaporator 2
    Y[4]=V2+L2-L1 #tot MB
    Y[5]=L2*xL2 - L1*xL1 #MB solids
    Y[6]=V2*HV2 + L2*hL2 - L1*hL1 - V1*(HV1-hk2) #EB för evaporator 2
    Y[7]=k2*A*((T1-B1)-T2) - V1*(HV1-hk2)      #EB för heat ex 2
    ...
    Y[11]=k3*A*((T2-B2)-T3) - V2*(HV2-hk3)       #EB för heat ex 3
...
guess=np.array([7,7,7,7,7,7,7,200,90,90,0.425,0.425]) #Set initial guesses
...
if not sol.success:
    ...
    print('Overall heat transfer coefficient for 1',k1,'kW/m^2*K')
    print('Overall heat transfer coefficient for 2',k2,'kW/m^2*K')
    print('Overall heat transfer coefficient for 3',k3,'kW/m^2*K')
    print('Beta 1',B1,'Celcius')
    print('Beta 2',B2,'Celcius')
    print('Beta 3',B3,'Celcius')
    print('Summa Beta',B1+B2+B3,'Celcius')

    [Hs,P,dummy]=H_steam(Ts,-1)
    [HV3,P,dummy]=H_steam(T3-B3,T3)
